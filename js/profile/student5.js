 // For Firebase JS SDK v7.20.0 and later, measurementId is optional
    ...
    async function getData() {
      ...
      const dosen = await db.collection("smt5").get();

      dosen.docs.forEach((mhs, i) => {
        const storageRef = storage.ref().child(`smt5/${mhs.data().nama}.jpg`);
    ...
    // TOTAL MAHASISWA
    function totalMhs() {
      db.collection('smt5').get().then(querySnapshot => {
    ...
    function mhsLaki() {
      db.collection('smt5').where('kelamin', 'in', ['Laki - laki']).get().then(querySnapshot => {
    ...
    function mhsPerempuan() {
      db.collection('smt5').where('kelamin', 'in', ['Perempuan']).get().then(querySnapshot => {
